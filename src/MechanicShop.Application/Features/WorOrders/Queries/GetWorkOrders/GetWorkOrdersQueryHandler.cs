    ...
    : IRequestHandler<GetWorkOrdersQuery, Result<PaginatedList<WorkOrderListItemDto>>>
{
    ...
    private static IQueryable<WorkOrder> ApplySearchTerm(IQueryable<WorkOrder> query, string searchTerm)
    {
        ...
        return query.Where(wo =>
            ...
            (wo.Labor != null && (
                (wo.Labor.FirstName ?? string.Empty).ToLower().Contains(normalized) ||
                (wo.Labor.LastName ?? string.Empty).ToLower().Contains(normalized) ||
                ((wo.Labor.FirstName ?? string.Empty) + " " + (wo.Labor.LastName ?? string.Empty)).ToLower().Contains(normalized)
            )) ||
            wo.RepairTasks.Any(rt =>
                rt.Name.ToLower().Contains(normalized)) ||
            wo.WorkOrderNumber.ToLower().Contains(normalized) ||
    ...
    private static IQueryable<WorkOrder> ApplySorting(IQueryable<WorkOrder> query, string sortColumn, string sortDirection)
    {
        ...
        {
            "workordernumber" => isDescending ? query.OrderByDescending(wo => wo.WorkOrderNumber) : query.OrderBy(wo => wo.WorkOrderNumber),
