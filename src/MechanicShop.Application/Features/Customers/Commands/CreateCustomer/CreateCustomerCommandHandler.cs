...
public class CreateCustomerCommandHandler(ILogger<CreateCustomerCommandHandler> logger, IAppDbContext context, HybridCache cache) : IRequestHandler<CreateCustomerCommand, Result<CustomerDto>>
{
    ...
    public async Task<Result<CustomerDto>> Handle(CreateCustomerCommand command, CancellationToken ct)
    {
        ...
        if (exists)
        {
            _logger.LogWarning("Customer creation rejected because email {Email} already exists", UtilityService.MaskEmail(email));
