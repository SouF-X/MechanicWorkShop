    ...
    : IRequestHandler<CreateRepairTaskCommand, Result<RepairTaskDto>>
{
    ...
    public async Task<Result<RepairTaskDto>> Handle(CreateRepairTaskCommand command, CancellationToken ct)
    {
        ...
        if (nameExists)
        {
            _logger.LogWarning("RepairTask creation rejected because name {RepairTaskName} already exists", command.Name);
        ...
        _logger.LogInformation("RepairTask {RepairTaskId} created successfully", repairTask.Id);
