    ...
    TimeProvider dateTime) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogDebug("Checking overdue work orders at {Now}", _dateTime.GetUtcNow());
            ...
            try
            {
                ...
                if (overdue.Count > 0)
                {
                    ...
                    foreach (var wo in overdue)
                    {
                        ...
                        {
                            wo.AddDomainEvent(new WorkOrderCollectionModified());
                        }

                        cancelledIds.Add(wo.Id);
                    }
                    ...
                    if (cancelledIds.Count > 0)
                    {
                        ...
                        _logger.LogInformation("Cancelled {Count} overdue work orders: {Ids}", cancelledIds.Count, cancelledIds);
                    }
                }
                else
                {
                    _logger.LogDebug("No overdue work orders found");
                }
            }
            catch (Exception ex)
            {
